    ...
    x_features = ['volt', 'rotate', 'pressure', 'vibration', 'cycle_comp1']
    ...
    df_wc['status'] = df_wc['replace_comp1'].map({True: False, False: True})
    df_wc['ttf_comp1'] = df_wc['ttf_comp1'] + 1


    # %% downsamples the data to balance the classes
    df_major = df_wc[df_wc['replace_comp1'] == False]               # não ocorre manutençoes
    df_minor = df_wc[df_wc['replace_comp1'] == True]                # manutençoes que acontecem
    ...
    time, survival_prob = kaplan_meier_estimator(df_sample["status"], df_sample["ttf_comp1"])
    ...
    def dataset_split(df, sample2test):
        # gets the limits
        idx_end = df[df['replace_comp1'] == True].index[sample2test]
        idx_start = df[df['replace_comp1'] == True].index[sample2test - 1] if sample2test != 0 else 0
        # computes the slices
        test_idx = (df.index > idx_start) & (df.index <= idx_end)
        x_train = df.loc[~test_idx, x_features].values
        y_train = np.array(list(zip(df.loc[~test_idx, 'status'], df.loc[~test_idx, 'ttf_comp1'])),
                        dtype=[('status', '?'), ('ttf_comp1', '<f8')])
        x_test = df.loc[test_idx, x_features].values
        y_test = np.array(list(zip(df.loc[test_idx, 'status'], df.loc[test_idx, 'ttf_comp1'])),
                        dtype=[('status', '?'), ('ttf_comp1', '<f8')])
    ...
    def plot_survival_function(model, x_test, y_test, idx2test):
        ...
        for i, surv_func in enumerate(pred_surv):
            plt.step(time_points, surv_func(time_points), where="post",
                    label="c={0}, ttf_comp1={1}".format(round(x_test[idx2test[i], -1]), int(y_test[idx2test[i]][1])))
        plt.ylabel("est. probability of survival $\hat{S}(t)$")
        plt.xlabel("cycles $c$")
        plt.legend(loc="best")
    ...
    def train_svm():
        ...
        cindex_train = concordance_index_censored(y_train['status'], y_train['ttf_comp1'], -svm_model.predict(x_train))
        print('cindex train:', round(cindex_train[0], 3))
        cindex_test = concordance_index_censored(y_test['status'], y_test['ttf_comp1'], -svm_model.predict(x_test))
    ...
    plt.xlabel("cycles $c$")
    plt.legend(loc="best")
    plt.title('Predicted time until replacement for Component 1.')
    plt.show()
    ...
    for ax, idx in zip(axes.flatten(), range(len(df_minor))):
        ...
        y_pred_sorted = y_pred[sorted_indices]
        y_test_sorted = y_test['ttf_comp1'][sorted_indices]
    ...
    plt.xlabel("cycles $c$")
    plt.legend(loc="best")
    plt.title('Predicted survival function for Component 1')
    plt.show()
# %%
dump(rsf, 'best_rsf_model_comp1.pkl')
dump(cox_model, 'best_coxph_model_comp1.pkl')
dump(gbsa, 'best_gbsa_model_comp1.pkl')
dump(svm_model, 'best_svm_model_comp1.pkl')
# %%
